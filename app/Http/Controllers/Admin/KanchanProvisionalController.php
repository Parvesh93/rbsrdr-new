...
use App\Models\KanchanProvisional;
use App\Models\Program;
use App\Models\Session;
use Illuminate\Http\Request;

class KanchanProvisionalController extends Controller
{
     /**
    ...
    {
        // Module Data
        $this->title = trans_choice('module_kanchan_provisional', 1);
        $this->route = 'admin.kanchan-provisional';
        $this->view = 'admin.kanchan-provisional';
        $this->path = 'kanchan-provisional';
        $this->access = 'kanchan-provisional';
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $kanchan_provisional = KanchanProvisional::where('status', '1');

                if (!empty($request->program)) {
                    $kanchan_provisional->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $kanchan_provisional->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $kanchan_provisional->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $kanchan_provisional->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $kanchan_provisional->orderby('id', 'desc')->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $kanchan_provisional = new KanchanProvisional();
            $kanchan_provisional->student_id = $request->student_id;
            $kanchan_provisional->registration_no = $request->registration_no;

            $kanchan_provisional->batch_id = $request->batch;
            $kanchan_provisional->program_id = $request->program;

            $kanchan_provisional->session_id = $request->session;
            $kanchan_provisional->name = $request->name;
            $kanchan_provisional->father_name = $request->father_name;
            $kanchan_provisional->mother_name = $request->mother_name;
            $kanchan_provisional->result = $request->result;
            $kanchan_provisional->date = $request->dob;

            $kanchan_provisional->save();
            $kanchan_provisional->serial_no = '000' . $kanchan_provisional->id;
     ...
     */
    public function edit(Request $request, KanchanProvisional $kanchan_provisional)
    {
        $data['title'] = $this->title;
        $data['route'] = $this->route;
        $data['view'] = $this->view;
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $kanchan_provisional;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $kanchan_provisional->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, KanchanProvisional $kanchan_provisional)
    {
     ...
     */
    public function destroy(KanchanProvisional $kanchan_provisional)
    {
        //Delete Data
        $kanchan_provisional->delete();
    ...
    public function provisional($id)
    {
        ...
        $data['rows'] = KanchanProvisional::where('id', $id)->orderBy('student_id', 'asc')->get();
