...
class BilashClcController extends Controller
{
    ...
    {
        // Module Data
        $this->title = trans_choice('module_bilash_clc', 1);
        $this->route = 'admin.bilash-clc';
        $this->view = 'admin.bilash-clc';
        $this->path = 'bilash-clc';
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $bilash_clc = BilashClc::where('status', '1');

                if (!empty($request->program)) {
                    $bilash_clc->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $bilash_clc->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $bilash_clc->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $bilash_clc->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $bilash_clc->orderby('id', 'desc')->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $bilash_clc = new BilashClc();
            $bilash_clc->student_id = $request->student_id;
            $bilash_clc->registration_no = $request->registration_no;

            $bilash_clc->batch_id = $request->batch;
            $bilash_clc->program_id = $request->program;

            $bilash_clc->session_id = $request->session;
            $bilash_clc->name = $request->name;
            $bilash_clc->father_name = $request->father_name;
            $bilash_clc->mother_name = $request->mother_name;
            $bilash_clc->division = $request->division;
            $bilash_clc->date = $request->dob;

            $bilash_clc->college_name = $request->college_name;

            $bilash_clc->save();
            $bilash_clc->serial_no = '000' . $bilash_clc->id;
     ...
     */
    public function edit(Request $request, BilashClc $bilash_clc)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $bilash_clc;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $bilash_clc->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, BilashClc $bilash_clc)
    {
     ...
     */
    public function destroy(BilashClc $bilash_clc)
    {
    ...
    public function clc($id)
    {
        ...
        $data['rows'] = BilashClc::where('id', $id)->orderBy('student_id', 'asc')->get();
