...
use App\Models\KanchanClc;
use App\Models\Program;
use App\Models\Session;
use Illuminate\Http\Request;

class KanchanClcController extends Controller
{
    public function __construct()
    {
        // Module Data
        $this->title = trans_choice('module_kanchan_clc', 1);
        $this->route = 'admin.kanchan-clc';
        $this->view = 'admin.kanchan-clc';
        $this->path = 'kanchan-clc';
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $kanchan_clc = KanchanClc::where('status', '1');

                if (!empty($request->program)) {
                    $kanchan_clc->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $kanchan_clc->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $kanchan_clc->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $kanchan_clc->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $kanchan_clc->orderby('id', 'desc')->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $kanchan_clc = new KanchanClc();
            $kanchan_clc->student_id = $request->student_id;
            $kanchan_clc->registration_no = $request->registration_no;

            $kanchan_clc->batch_id = $request->batch;
            $kanchan_clc->program_id = $request->program;

            $kanchan_clc->session_id = $request->session;
            $kanchan_clc->name = $request->name;
            $kanchan_clc->father_name = $request->father_name;
            $kanchan_clc->mother_name = $request->mother_name;
            $kanchan_clc->division = $request->division;
            $kanchan_clc->date = $request->dob;

            $kanchan_clc->college_name = $request->college_name;

            $kanchan_clc->save();
            $kanchan_clc->serial_no = '000' . $kanchan_clc->id;
     ...
     */
    public function edit(Request $request, KanchanClc $kanchan_clc)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $kanchan_clc;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $kanchan_clc->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, KanchanClc $kanchan_clc)
    {
     ...
     */
    public function destroy(KanchanClc $kanchan_clc)
    {
    ...
    public function clc($id)
    {
        ...
        $data['rows'] = KanchanClc::where('id', $id)->orderBy('student_id', 'asc')->get();
