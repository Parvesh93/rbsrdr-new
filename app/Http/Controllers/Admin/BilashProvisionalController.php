...
use App\Models\BilashProvisional;
...
class BilashProvisionalController extends Controller
{
    /**
    ...
    {
        // Module Data
        $this->title = trans_choice('module_bilash_provisional', 1);
        $this->route = 'admin.bilash-provisional';
        $this->view = 'admin.bilash-provisional';
        $this->path = 'bilash-provisional';
        $this->access = 'bilash-provisional';
    ...
    public function index(Request $request)
    {
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            if (isset($request->session) || isset($request->program)  || isset($request->student_id) || isset($request->student_regi)) {
                ...
                $bilash_provisional = BilashProvisional::where('status', '1');

                if (!empty($request->program)) {
                    $bilash_provisional->where('program_id', $program);
                }
                if (!empty($request->session)) {
                    $bilash_provisional->where('session_id', $session);
                }
                if (!empty($request->student_id)) {
                    $bilash_provisional->where('student_id', 'like', '%' . $student_id . '%');
                }
                if (!empty($request->student_regi)) {
                    $bilash_provisional->where('registration_no', 'like', '%' . $student_regi . '%');
                }




                $data['rows'] = $bilash_provisional->orderby('id', 'desc')->get();
            }
    ...
    public function store(Request $request)
    {
        ...
        try {
            //Data Insert
            $bilash_provisional = new BilashProvisional();
            $bilash_provisional->student_id = $request->student_id;
            $bilash_provisional->registration_no = $request->registration_no;

            $bilash_provisional->batch_id = $request->batch;
            $bilash_provisional->program_id = $request->program;

            $bilash_provisional->session_id = $request->session;
            $bilash_provisional->name = $request->name;
            $bilash_provisional->father_name = $request->father_name;
            $bilash_provisional->mother_name = $request->mother_name;
            $bilash_provisional->result = $request->result;
            $bilash_provisional->date = $request->dob;

            $bilash_provisional->save();
            $bilash_provisional->serial_no = '000' . $bilash_provisional->id;
     ...
     */
    public function edit(Request $request, BilashProvisional $bilash_provisional)
    {
        $data['title'] = $this->title;
        $data['route'] = $this->route;
        $data['view'] = $this->view;
        ...
        if (auth()->user()->is_admin === 1) {
            ...
            $data['row'] = $bilash_provisional;
            $teacherDepa = auth()->user()->teacher_department; // ya ->faculty_name
            $program_id = $bilash_provisional->program_id; // ya ->faculty_name
            $collegeDepartment = auth()->user()->college_department_id;
     ...
     */
    public function update(Request $request, BilashProvisional $bilash_provisional)
    {
     ...
     */
    public function destroy(BilashProvisional $bilash_provisional)
    {
        //Delete Data
        $bilash_provisional->delete();
    ...
    public function provisional($id)
    {
        ...
        $data['rows'] = BilashProvisional::where('id', $id)->orderBy('student_id', 'asc')->get();
