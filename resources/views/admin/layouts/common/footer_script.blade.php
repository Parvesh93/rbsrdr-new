                    ...
                    exportOptions: {
                        ...
                        format: {
                            body: function(data, row, column, node) {
                                // Remove all HTML and return plain text (with line breaks handled)
                                if (typeof data === 'string' && data.indexOf('<') !== -1) {
                                    // Replace <span> or other inline tags with space
                                    const html = $('<div>').html(data);
                                    html.find('br').replaceWith('\n');
                                    html.find('span, div').each(function() {
                                        $(this).after(' ');
                                    });
                                    return html.text().trim().replace(/\s+/g, ' ');
                                }
                                return data;
                            }
                        }
